  ...
  return (
    <>
      <box flexDirection="row" flexGrow={1} width="100%" height="100%">
        {/* SIDEBAR compact fit white text */}
        <Show when={wide()}>
          <box width={30} flexShrink={0} flexDirection="column" backgroundColor={theme.backgroundPanel} border={["right"]} borderColor={theme.borderSubtle} paddingLeft={1} paddingRight={1} paddingTop={0} paddingBottom={0} gap={0}>
            ...
            <box flexDirection="column" gap={0} paddingLeft={1} paddingRight={1} paddingTop={1}>
              <box marginLeft={1} backgroundColor={theme.background} border={["top","bottom","left","right"]} borderColor={theme.background} customBorderChars={Rounded} paddingLeft={1} paddingRight={1} paddingTop={0} paddingBottom={0}>
            ...
            <box flexDirection="column" gap={0} marginTop={1} flexGrow={1}>
              ...
              <box flexDirection="column" gap={0} flexGrow={1} marginTop={1}>
                <For each={recentSessions()}>{(sess, idx) => (
                  <box flexDirection="column" gap={0} paddingLeft={1} paddingRight={1} paddingTop={0} paddingBottom={1} backgroundColor={idx() % 2 === 0 ? theme.backgroundPanel : theme.background} onMouseUp={() => router.navigate({ type: "session", sessionID: sess.id })}>
                    <box flexDirection="row" gap={1}>
                      <text fg={theme.textMuted}>Session</text>
                      <text fg={theme.text}>{((sess as any).title || "New session").toString().slice(0, 20)} - {formatSessionDate(sess.time.updated)}</text>
                    </box>
                    <box flexDirection="row" gap={1} marginTop={1}>
                      <text fg={theme.textMuted}>Continue</text>
                      <text fg={theme.text}>opencode -s {sess.id}</text>
                    </box>
                  </box>
                )}</For>
              </box>
        ...
        {/* MAIN - OLD BANNER + RESTORE SESSION LIKE EXAMPLE with ARENA CODE ASCII */}
        <box flexGrow={1} flexDirection="column" justifyContent="center" alignItems="center" paddingLeft={1} paddingRight={1} gap={0}>
          <box width="100%" maxWidth={80} flexDirection="column" gap={0} alignItems="center" justifyContent="center" flexGrow={1}>
            {/* OLD BANNER COMPACT - ARENA CODE ASCII hehe */}
            ...
            <box width="100%" maxWidth={70} flexDirection="column" gap={0} zIndex={1000} marginTop={1}>
              ...
              {/* RESTORE SESSION LIKE EXAMPLE - ARENA CODE ASCII */}
              <Show when={recentSessions().length > 0}>
                <box flexDirection="column" gap={0} marginTop={2} width="100%" alignItems="center">
                  <box width="100%" maxWidth={70} flexDirection="column" gap={1} backgroundColor={theme.backgroundPanel} border={["top","bottom","left","right"]} borderColor={theme.borderSubtle} customBorderChars={Rounded} paddingLeft={2} paddingRight={2} paddingTop={1} paddingBottom={1}>
                    <For each={recentSessions()}>{(sess) => (
                      <box flexDirection="column" gap={0} marginBottom={1}>
                        <box flexDirection="row" gap={2}>
                          <text fg={theme.textMuted}>Session</text>
                          <text fg={theme.text}>{((sess as any).title || "New session")} - {formatSessionDate(sess.time.updated)}</text>
                        </box>
                        <box height={1} />
                        <box flexDirection="row" gap={1}>
                          <text fg={theme.textMuted}>Continue</text>
              ...
              <Show when={!wide() && recentSessions().length === 0}>
                <box flexDirection="column" gap={0} marginTop={2} width="100%" alignItems="center">
                  <text fg={theme.textMuted}>○ No sessions — start new chat ○</text>
                </box>
              </Show>
            </box>
          </box>

          <box width="100%" maxWidth={70} flexDirection="row" gap={1} paddingTop={0} paddingBottom={0} flexShrink={0} justifyContent="center">
            <text fg={theme.textMuted}>○ {versionText} · restore session · arena ascii · fit</text>
          </box>
          <Toast />
        </box>
      </box>
