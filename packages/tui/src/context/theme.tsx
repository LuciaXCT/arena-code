  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "arena-code")
        draft.active = typeof active === "string" ? active : "arena-code"
        draft.ready = false
      }),
    )
    ...
    function syncCustomThemes() {
      return themes
        ...
        .catch(() => setStore("active", "arena-code"))
    }
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "arena-code")
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes["arena-code"] ?? store.themes.opencode, store.mode)
    })
