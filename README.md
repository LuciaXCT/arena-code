...
**Package:** `@pawbxj/arena-cli` • **Command:** `arena` • **Version:** `0.0.8-dev` • **Author:** Pawbxj
...
npm run clean
npm run build
./dist/cli.js --version  # must be 0.0.8-dev
./dist/cli.js --help
...
npm pack
# -> pawbxj-arena-cli-0.0.8-dev.tgz

# Test with npm
npm install -g ./pawbxj-arena-cli-0.0.8-dev.tgz
arena --version
arena --help
arena "test task"
...
bun install -g ./pawbxj-arena-cli-0.0.8-dev.tgz
arena --version
bun pm ls -g | grep arena
bunx @pawbxj/arena-cli --help

# Also test npx/bunx without global install (from the tarball)
npx --package ./pawbxj-arena-cli-0.0.8-dev.tgz -c "arena --help"
bunx --package ./pawbxj-arena-cli-0.0.8-dev.tgz arena --help
...
`0.0.8-dev` (pre-release, ESM, Node `>=18`, Bun `>=1.0`)
